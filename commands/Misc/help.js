...
async function help(message, args) {
    ...
    if (args.length === 0) {
        ...
        .setDescription(
            ...
            `Chọn vào mục bên dưới để xem tất cả cách lệnh sãn có`
        );

        categories.forEach((cate, ) => {
            let _name = cate.charAt(0).toUpperCase() + cate.slice(0).toLowerCase()
            let emoji = emojis[cate] || ''
            let name = emoji + " " + _name;
            let desc = description[cate] ? description[cate] : _name;
            ...
            menuOptions.push(
                { // push the menu select options for each category into the "menuOptions" array
                    label: `${cate}`, // label of the select menu option
                    description: `${cate} commands page`, // description of the select menu option
                    emoji: emoji, // emoji of the select menu option
        ...
        .addComponents( // create a new MessageActionRow and add components
        ...
        ); // send the embed with await for edit embed later
